...
var chartsid = ['leftchart1', 'leftchart2', 'leftchart3', 'rightchart2'];
...
inityseriesname.leftchart3 = [
	{name : 'peopledistinctnb', label : 'Comptes actifs'},
	{name : 'peoplenb', label : 'Connexions'}
	//{name : 'total_accounts', label : 'Nombre total de comptes'}
];
inityseriesname.rightchart2 = [
	{name : 'bookloads', label : 'Livres consultés'},
	{name : 'bookfolders', label : 'Livre ajouté'},
	//{name : 'bookallfolders', label : "Nombre total d'étagères"},
...
function ready(stats) {
	...
	// ROLES = FUNCTIONS
	rolesidname = {};
	datastats.roles.id.map(function(d, i){
		rolesidname[datastats.roles[functionname][i]] = d;
	});
	console.log(rolesidname);
	...
	datastats.covers.docid.map(function(d, i){
 	...
 	console.log(docidcoversimg);

	// LIST OF SCHOOL NAMES
	d3
		.select('#functionlist')
		.selectAll('option')
		.data(stats.roles[functionname].map(function(d){ return d; }))
		.enter()
		.append('option')
	...
	d3
		...
		.on('change', function(d){
			selectedfct = rolesidname[d3.select('#functionlist').node().value];
			whichseries = 'loads';
	...
	yseriesname[id_of_the_chart].map(function(d){ 
		...
		if(whichpane == 'leftpane'){	
			...
			datastats.seriesdocid.docid
				.map(function(e, i){ if(e == docidselected & datastats.seriesdocid.functionid[i] == selectedfct){ 
					tempindexes.push(i) 
				}});
			tempindexes.map(function(e){
				allseries.push({'statdate': timeParse(datastats.seriesdocid.statdate[e]), 
					'datefct': datastats.seriesdocid.statdate[e].toString() + datastats.seriesdocid.functionid[e].toString(),
...
*/
	...
	glegendenter
		...
		.on('click', function(d){		
			...
			//rolesidname[d3.select('#functionlist').node().value],
